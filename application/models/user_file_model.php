...
class User_file_model extends CI_Model{
	...
	function get_recent_activities($email){
		...
		foreach($query->result_array() as $row){
			array_push($activities, array('activity_id'=> $row['activity_id'], 'activity_date' => $row['activity_date'], 'activity_name' => $row['activity_name'], 'activity_type' => $row['activity_type'], 'standard_ride_id' => $row['standard_ride_id']));
		}
		return $activities;
	}

	function has_activities($email){
		$query = $this->db->query("SELECT activity_id FROM user_activity WHERE email = '$email'  LIMIT 1");
		$num_activities = count($query->result_array());
