...
class SendVerification extends CI_Controller {
	...
	function index()
	{
		...
		//send out validation email
                ...
                $this->email->message('Hi '.$this->username.'. Please click this link to validate your email. '.$this->config->item('base_url').'/validate?'.do_hash($this->config->item('salt').this->db->insert_id()));  
                $this->email->send();
